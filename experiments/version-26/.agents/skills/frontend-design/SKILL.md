...
- No clipped headline text at mobile widths (375-390px); force wrapping and tune clamps until every key headline is fully readable
- Google Fonts with `display=swap` and preconnect
- CSS custom properties for all colors

---

## Phase 11: Depth Gates (Non-Negotiable)

Hero quality is not enough. Every route must have below-the-fold structure.

- Each route must contain **at least 4 vertical sections**:
  - 1 hero section
  - 3 additional sections after hero
- The first post-hero section must start immediately after hero, and at least one section must appear clearly beyond first fold depth.
- Required section types per route:
  - one **proof/information** section (logos, stats, comparison, process, or evidence)
  - one **interactive** section below hero (tabs/toggle/accordion/filter that changes visible content)
  - one **closing CTA** section near the end of the page
- Do not end a route after a hero + single support block.
- Do not repeat the same section template three times; section archetypes must vary.

Preflight self-check before finalizing:
1. Count sections per route (must be 4+).
2. Confirm at least one interactive module is below hero, not only in hero.
3. Confirm no headline clipping at 375-390px mobile width.
