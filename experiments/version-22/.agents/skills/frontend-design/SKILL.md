...
## Style Family Lock and Typographic Identity Guard

Treat default visual fallback as a failure:

- Before implementation, assign one style family to each concept and lock it for the full concept:
  - Editorial Print: high-contrast serif display, restrained sans body, warm paper-leaning light palette.
  - Neo Grotesk Command: grotesk-led system with medium/heavy weights, dark neutrals plus one electric accent.
  - Soft Product Calm: rounded humanist sans, soft tonal contrast, low-noise surfaces.
  - Brutalist Utility: condensed or heavy display, hard edges, strict borders, blunt high-contrast hierarchy.
  - Retro Tech Terminal: mono-forward accents, supporting sans body, scanline/grid atmosphere, phosphor or amber highlights.
- Do not mix more than one family in a single concept.
- In multi-concept sets, do not reuse a style family across concepts unless required by user constraints.
- Declare a type hierarchy plan per concept before styling:
  - hero size/weight
  - section title size/weight
  - body size/weight
- Do not reuse the same hero/title/body weight pattern in more than one concept.
- Assign a color temperature intent per concept (warm, cool, neutral, pastel, or high-contrast) and avoid repeating the same intent in adjacent concepts.
- Keep accent usage disciplined: one primary accent and at most one support accent unless the family explicitly demands more.
- If two concepts still feel typographically or chromatically similar after stripping decorative effects, rework one with a different family lock.
- Keep family names internal; user-facing copy should remain product-focused.
